...
VibeCast 使用目标 Profile 管理 App 的激活、聚焦、写入和发送行为。Codex、WorkBuddy、Notion、Obsidian、CodeBuddyCN、CodeBuddy 是预置目标；配置页也可以添加任意自定义 App。
...
2. 勾选需要在手机端显示的预置目标；不需要的目标可停用。首次安装时，已安装的预置 App 会自动启用。
3. 预置目标已内置 Bundle ID；自定义目标可在“快速选择”里选择运行中的应用，自动填入 Bundle ID。
...
| `syncMode` | 同步语义：`mirror` 完整草稿镜像；`editor` 只替换本轮插入文本 |
...
| 类型 | 行为 |
|---|---|
| 预置目标 | 默认存在；内置 Bundle ID；首次安装时如检测到本机已安装则自动启用；可启用/停用；不可删除 |
...
| 策略 | 行为 | 适用场景 |
|---|---|---|
| `mirror` | 手机草稿作为完整目标内容写入；修改、删除会同步到目标输入框 | 聊天输入框、Notion AI 输入框、原生单输入框 |
| `editor` | 首次在当前光标/选区插入，后续只替换本轮由 VibeCast 插入的文本段；不会执行整页全选 | Obsidian、Notion 普通文档块、富文本编辑器 |

`editor` 模式依赖目标应用暴露可读写的 Accessibility 文本选区。若 VibeCast 无法确认本轮插入段，会返回写入失败，而不会退回 `Cmd+A` 全选替换。
...
| 目标 | 建议 |
|---|---|
| Codex / WorkBuddy / CodeBuddyCN / CodeBuddy | `focusMode=shortcut`，配置聊天框聚焦快捷键；`writeMode=auto`；`sendMode=key`；`sendShortcut=enter` |
...
| Notion 当前文本块 | `focusMode=preserve_last_focus`；`syncMode=editor`；`writeMode=clipboard_insert`；`sendMode=none`；`allowSelectAllReplace=false` |
| Obsidian | 预置默认 `syncMode=editor`；先手动点好编辑器光标，手机端“完成”后保留 Mac 文本并清空本轮手机输入 |
