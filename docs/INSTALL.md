...
发布包使用本地签名（ad-hoc 或 `CODESIGN_IDENTITY` 指定的本机证书）。首次打开时如果 macOS 安全提示拦截，请在系统设置中允许本地应用运行。
...
如果系统设置里 VibeCast 的开关已经打开，但配置页或菜单栏仍显示“当前运行版本未授权”，通常是本地重新打包后 macOS TCC 仍保留旧签名记录。请在系统设置中将 VibeCast 关闭再打开，必要时退出并重新打开 VibeCast。
