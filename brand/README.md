# VibeCast Logo

## Concept

The VibeCast mark combines three product ideas:

- Voice waveform: the three rounded bars on the left.
- Local casting: the rounded input field carries the signal across devices.
- Text cursor: the dark vertical mark on the right represents mirrored text ready to send.

## Palette

- Focus teal: `#0D9488`
- Vibe rose: `#E11D48`
- Cast blue: `#2563EB`
- Ink: `#0F172A`
- White: `#FFFFFF`

## Assets

- `vibecast-app-icon.svg` is the editable source for the square app icon.
- `vibecast-logo.svg` is the horizontal brand lockup for docs and presentation use.

Runtime exports live in:

- `web/public/` for browser, PWA, and home-screen icons.
- `mac/Sources/VibeCast/Resources/AppIcon.icns` for the macOS bundle.
