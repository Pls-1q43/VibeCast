...
enum AXSupport {
    ...
    /// 在某进程菜单栏中查找标题匹配的菜单项并按下。用于触发系统/应用菜单命令。
    static func pressMenuItem(pid: pid_t, titleMatches: [String], maxDepth: Int = 8) -> Bool {
        let app = AXUIElementCreateApplication(pid)
        guard let menuBar = attribute(app, kAXMenuBarAttribute) else { return false }
        guard let menus = children(of: menuBar) else { return false }

        for menu in menus {
            _ = AXUIElementPerformAction(menu, kAXPressAction as CFString)
            Thread.sleep(forTimeInterval: 0.06)
            if let item = findMenuItem(in: menu, titleMatches: titleMatches, depth: maxDepth),
    ...
    private static func findMenuItem(in element: AXUIElement, titleMatches: [String], depth: Int) -> AXUIElement? {
        if depth < 0 { return nil }
        let title = stringAttr(element, kAXTitleAttribute) ?? stringAttr(element, kAXDescriptionAttribute) ?? ""
        if role(of: element) == (kAXMenuItemRole as String),
           titleMatches.contains(where: { title.localizedCaseInsensitiveContains($0) }),
           isEnabled(element) {
            return element
        }
        guard let children = children(of: element) else { return nil }
        for child in children {
            if let found = findMenuItem(in: child, titleMatches: titleMatches, depth: depth - 1) {
                return found
            }
        }
        return nil
    ...
    private static func attribute(_ element: AXUIElement, _ attr: String) -> AXUIElement? {
        var value: CFTypeRef?
